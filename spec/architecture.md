...
lund/*.{jpg,jpeg,webp,avif,cms}
  ...
  tools/curate.js ─────────► tools/out/target-set.json
   size → dedup → cluster       41 target embeddings
   → outlier trim
        │
        │        LFW ──► tools/negatives.js ──► tools/out/negatives.json
        │                 one image per identity      5,187 strangers
        ▼                          │
  tools/calibrate.js ◄─────────────┘
   leave-one-out recall vs. measured stranger FPR,
   scored on a held-out half of the negatives
        │
        ▼
  extension/public/targets.json   ← 41 embeddings + threshold 0.445
        │
        ▼
  tools/audit.js
   re-test the threshold on full-resolution Indian faces
```

Each stage is an npm script: `embed`, `curate`, `negatives`, `calibrate`, `audit`.
...
On the 47 source photos this kept 41 embeddings: 2 faces were too small, 3 were
duplicates, and 2 were bystanders in a group shot.
...
false-positive rate against a large stranger set.

Three traps the pipeline guards against, each of which silently flatters the
result if ignored:

- **The target inside the negative set.** LFW contains `Narendra_Modi`, so his own
  portrait scored as the nearest "stranger". Excluded by identity, and reported.
- **Fitting and grading on the same data.** The margin rule picks its own hard
  negatives, so negatives are split into fit and holdout halves.
- **Self-distance in the margin test.** A negative that is itself in the hard set
  scores 0 against it and gets rejected for free. Same-identity vectors are
  skipped when measuring that distance.
...
**As shipped:** `threshold 0.445`, `margin 0`, `minFacePx 80`, 41 target vectors,
no bundled negatives — 100% leave-one-out recall with 1 false positive in 5,187
strangers. Step 4 is inert while `margin` is 0; the code path stays because
re-calibrating on a larger negative set may switch it on.

See [00-first-spec.md](00-first-spec.md) for how 0.445 was arrived at.
...
- Model weights live in `public/` and are loaded via `chrome.runtime.getURL()`.
  ...
  a hashed filename that cannot be referenced. The build output is checked to
  confirm they land as real files at predictable paths.
